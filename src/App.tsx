...
import { Search, TrendingUp, Activity, BarChart3, RefreshCw, ChevronRight } from "lucide-react";
...
export default function App() {
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      await fetch("/api/stocks/sync", { method: "POST" });
      await fetchStocks();
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans p-6">
      <header className="max-w-7xl mx-auto mb-12 flex justify-between items-end">
        ...
        >
          ...
          <span className="text-xs font-bold uppercase tracking-tighter">Sync Market Data</span>
        </button>
      </header>
